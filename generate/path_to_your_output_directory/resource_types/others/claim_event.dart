/// /// [Claim_Event] A provider issued list of professional services and
/// products which have been provided, or are to be provided, to a patient
/// which is sent to an insurer for reimbursement.

@freezed
class Claim_Event with _$Claim_Event {
  const Claim_Event._();

  const factory Claim_Event({
...
/// event occurred or is expectd to occur.
  ...
  @override
  String get fhirType => 'Claim_Event';

  factory Claim_Event.fromJson(Map<String, dynamic> json) =>
      _$Claim_EventFromJson(json);

  factory Claim_Event.fromYaml(dynamic yaml) => yaml is String
      ? Claim_Event.fromJson(
          jsonDecode(jsonEncode(loadYaml(yaml))) as Map<String, dynamic>)
      : yaml is YamlMap
          ? Claim_Event.fromJson(jsonDecode(jsonEncode(yaml)) as Map<String, dynamic>)
          : throw ArgumentError(
              'Claim_Event cannot be constructed from input provided,'
              ' it is neither a yaml string nor a yaml map.');

  factory Claim_Event.fromJsonString(String source) {
    final dynamic json = jsonDecode(source);
    if (json is Map<String, dynamic>) {
      return _$Claim_EventFromJson(json);
    } else {
