...
import '../../r4.dart';
/// [TestScript_Teardown] A structured set of tests against a FHIR server
/// or client implementation to determine compliance against the FHIR
/// specification.

@freezed
class TestScript_Teardown with _$TestScript_Teardown {
  const TestScript_Teardown._();

  const factory TestScript_Teardown({
...
/// [action] The teardown action will only contain an operation.
    ...
    @JsonKey(name: 'action') List<List<TestScript_Action2>>? action,
  }) = _$TestScript_Teardown;

  @override
  String get fhirType => 'TestScript_Teardown';
  ...
  factory TestScript_Teardown.fromYaml(dynamic yaml) => yaml is String
      ...
      : yaml is YamlMap
          ? TestScript_Teardown.fromJson(jsonDecode(jsonEncode(yaml)) as Map<String, dynamic>)
          : throw ArgumentError(
