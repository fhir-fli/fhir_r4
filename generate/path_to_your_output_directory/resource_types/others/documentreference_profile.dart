...
import '../../r4.dart';
/// [DocumentReference_Profile] A reference to a document of any kind for
...
@freezed
class DocumentReference_Profile with _$DocumentReference_Profile {
  const DocumentReference_Profile._();

  const factory DocumentReference_Profile({
...
/// [_valueCanonical] Extensions for valueCanonical
  ...
  }) = _$DocumentReference_Profile;

  @override
  String get fhirType => 'DocumentReference_Profile';

  factory DocumentReference_Profile.fromJson(Map<String, dynamic> json) =>
      _$DocumentReference_ProfileFromJson(json);

  factory DocumentReference_Profile.fromYaml(dynamic yaml) => yaml is String
      ? DocumentReference_Profile.fromJson(
          jsonDecode(jsonEncode(loadYaml(yaml))) as Map<String, dynamic>)
      : yaml is YamlMap
          ? DocumentReference_Profile.fromJson(jsonDecode(jsonEncode(yaml)) as Map<String, dynamic>)
          : throw ArgumentError(
              'DocumentReference_Profile cannot be constructed from input provided,'
              ' it is neither a yaml string nor a yaml map.');

  factory DocumentReference_Profile.fromJsonString(String source) {
    final dynamic json = jsonDecode(source);
    if (json is Map<String, dynamic>) {
      return _$DocumentReference_ProfileFromJson(json);
    } else {
