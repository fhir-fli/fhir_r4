...
@freezed
class StructureMap with _$StructureMap {
...
/// structure map when it is represented in other formats, or referenced in a
...
/// definition may describe instances that are converted, or the instances that
/// are produced.

    @JsonKey(name: 'structure') List<List<StructureMap_Structure>>? structure,
/// [import] Other maps used by this map (canonical URLs).

    @JsonKey(name: 'import') List<List<FhirCanonical>>? import,
/// [const] Definition of a constant value used in the map rules.

    @JsonKey(name: 'const') List<List<StructureMap_Const>>? const,
/// [group] Organizes the mapping into managable chunks for human
/// review/ease of maintenance.

    @JsonKey(name: 'group') List<List<StructureMap_Group>>? group,
  }) = _$StructureMap;
  ...
  factory StructureMap.fromYaml(dynamic yaml) => yaml is String
      ...
      : yaml is YamlMap
          ? StructureMap.fromJson(jsonDecode(jsonEncode(yaml)) as Map<String, dynamic>)
          : throw ArgumentError(
