...
Future<void> testOfficialGaps() async {
  ...
  group("official-comments", () {
    ...
    test("testComment9", () async {
      expect(
        await eval(patient1, "2 + /* inline \$@%^+ * */ 2 = 4"),
        ["true"],
      );
    });
  ...
  group("official-testTypes", () {
    ...
    test("testStringQuantityMonthLiteralToQuantity", () async {
      expect(
        await eval(patient1, "'1 \\'mo\\''.toQuantity() = 1 month"),
        [],
      );
    });
    ...
    test("testStringQuantityYearLiteralToQuantity", () async {
      expect(
        await eval(patient1, "'1 \\'a\\''.toQuantity() = 1 year"),
        [],
      );
    });
  ...
  group("official-testSort", () {
    ...
    test("testSort3", () async {
      expect(
        await eval(patient1, "(1 | 2 | 3).sort(\$this) = (1 | 2 | 3)"),
        ["true"],
      );
    });
    ...
    test("testSort4", () async {
      expect(
        await eval(patient1, "(3 | 2 | 1).sort(\$this) = (1 | 2 | 3)"),
        ["true"],
      );
    });
    ...
    test("testSort5", () async {
      expect(
        await eval(patient1, "(1 | 2 | 3).sort(-\$this) = (3 | 2 | 1)"),
        ["true"],
      );
    });
  ...
  group("official-testUnion", () {
    ...
    test("testUnion10", () async {
      expect(
        await eval(patient1, "name.select(use.union(\$this.given)).count()"),
        ["8"],
      );
    });
