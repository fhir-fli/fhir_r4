...
| 3.1.1.4.10.1 | `contenttype:below=text/xml` finds `text/xml; charset=UTF-8`; "only required to support :below on the base part of the mime type" | ✅ token `:below` on a value with a `/` is a mime-base match; on a code it is subsumption and refused | ":below on a mime type and on a canonical reference" |
...
| 3.1.1.4.13 | versioned references `Observation/123/_history/234`: search acts on the current version; SHOULD warn with an OperationOutcome | ⚠️ type+id match ignores the version, as the text allows; the warning is ⛔ the server's | — |
| 3.1.1.4.13 | canonical `\|[version]`; `:below` "to control whether the version is considered": `…/p` matches `\|1.0`, `\|1.1`, `\|2.0`; `…/p\|1` the first two | ✅ (was: `:below` on a reference refused / answered as a type named "below") | ":below on a mime type and on a canonical reference" |
