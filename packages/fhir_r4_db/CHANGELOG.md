...
- 🛑 **The reference resolver is deliberately NOT converted.** With it, `subject=Patient/does-not-exist` went from 0.01s to 10.35s: the conversion stopped that query using an index. A thousandfold regression on the common case is not worth the marshalling saved, and why it happens is not yet understood.
