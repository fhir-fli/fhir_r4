...
void testBasicTypes() {
  group('Basic Types', () {
    test('Boolean', () {
      expect(
        (((parseResult('true') as ParserList)).first as ValueParser).value,
    ...
    test('String', () {
      expect(
        (((parseResult("'test string'") as ParserList)).first as ValueParser)
            .value,
        (ParserList([StringParser("test string")]).first as ValueParser).value,
      );
      expect(
        (((parseResult("'urn:oid:3.4.5.6.7.8'") as ParserList)).first
                as ValueParser)
            .value,
        (ParserList([StringParser("urn:oid:3.4.5.6.7.8")]).first as ValueParser)
            .value,
      );
    });
    ...
    test('Integer', () {
      expect(
        (((parseResult('0') as ParserList)).first as ValueParser).value,
        (ParserList([IntegerParser(0)]).first as ValueParser).value,
      );
      expect(
        (((parseResult('45') as ParserList)).first as ValueParser).value,
    ...
    test('Decimal', () {
      expect(
        (((parseResult('0.0') as ParserList)).first as ValueParser).value,
        (ParserList([DecimalParser(0)]).first as ValueParser).value,
      );
      expect(
        (((parseResult('3.1415927') as ParserList)).first as ValueParser).value,
    ...
    test('Date', () {
      expect(
        (((parseResult('@2015-02-04') as ParserList)).first as ValueParser)
            .value,
        (ParserList([DateParser(FhirDate('2015-02-04'))]).first as ValueParser)
            .value,
      );
      expect(
        (((parseResult('@2018-06-12') as ParserList)).first as ValueParser)
            .value,
        (ParserList([DateParser(FhirDate('2018-06-12'))]).first as ValueParser)
            .value,
      );
      expect(
        (((parseResult('@2018-06-12') as ParserList)).first as ValueParser)
      ...
      expect(
        (((parseResult('@2018-06') as ParserList)).first as ValueParser).value,
        (ParserList([DateParser(FhirDate('2018-06'))]).first as ValueParser)
            .value,
      );
      expect(
        (((parseResult('@2018') as ParserList)).first as ValueParser).value,
        (ParserList([DateParser(FhirDate('2018'))]).first as ValueParser).value,
      );
    });
    test('DateTime', () {
      expect(
        (((parseResult('@2015-02-04T14:34:28+09:00') as ParserList)).first
            ...
            .value,
        (ParserList([
          DateTimeParser(FhirDateTime(DateTime.utc(2015, 2, 4, 5, 34, 28))),
      ...
      expect(
        (((parseResult('@2014-01-25T14:30:14.559') as ParserList)).first
                as ValueParser)
            .value,
        (ParserList([DateTimeParser(FhirDateTime('2014-01-25T14:30:14.559'))])
                .first as ValueParser)
            .value,
      );
      expect(
      ...
      expect(
        ((parseResult('@2014-01-25T14:30 // A partial DateTime with year, month, day, hour, and minute')
                ...
                .first as ValueParser)
            .value,
        (ParserList([DateTimeParser(FhirDateTime('2014-01-25T14:30'))]).first
                as ValueParser)
            .value,
      );
      expect(
        ((parseResult('@2014-03-25T // A partial DateTime with year, month, and day')
                ...
                .first as ValueParser)
            .value,
        (ParserList([DateTimeParser(FhirDateTime('2014-03-25'))]).first
                as ValueParser)
            .value,
      );
      expect(
        ((parseResult('@2014-01T // A partial DateTime with year and month')
                ...
                .first as ValueParser)
            .value,
        (ParserList([DateTimeParser(FhirDateTime('2014-01T'))]).first
                as ValueParser)
            .value,
      );
      expect(
        ((parseResult('@2014T // A partial DateTime with only the year')
                ...
                .first as ValueParser)
            .value,
        (ParserList([DateTimeParser(FhirDateTime('2014T'))]).first
                as ValueParser)
            .value,
      );
    });
    ...
    test('Quantity', () {
      expect(
        ...
        QuantityParser(
          ValidatedQuantity(
            value: UcumDecimal.fromString('4.5'),
            unit: 'mg',
          ),
        ),
      );
    ...
    test('Duration quantities', () {
      expect(
        walkFhirPath(context: null, pathExpression: "1 seconds = 1 second"),
        [true],
      );
      expect(
        walkFhirPath(context: null, pathExpression: "1 seconds = 1 's'"),
        [true],
      );
      expect(
        walkFhirPath(context: null, pathExpression: "2 seconds = 2 's'"),
        [true],
      );
      expect(
        walkFhirPath(context: null, pathExpression: "1 week != 1 'wk'"),
        [false],
      );
    });
    test('Non-Escape Sequences', () {
      ...
      expect(
        ((parseResult("'\' // ''") as ParserList).first as StringParser).value,
        StringParser('').value,
      );
    });
  });
