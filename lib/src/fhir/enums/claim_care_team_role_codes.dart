/// This value set includes sample Claim Care Team Role codes.
enum ClaimCareTeamRoleCodes {
  /// Display: Primary provider
  /// Definition: The primary care provider.
  primary,

  /// Display: Assisting Provider
  /// Definition: Assisting care provider.
  assist,

  /// Display: Supervising Provider
  /// Definition: Supervising care provider.
  supervisor,

  /// Display: Other
  /// Definition: Other role on the care team.
  other,
  ;

  @override
  String toString() {
    switch (this) {
      case primary:
        return 'primary';
      case assist:
        return 'assist';
      case supervisor:
        return 'supervisor';
      case other:
        return 'other';
    }
  }

  /// Returns a [String] from a [ClaimCareTeamRoleCodes] enum.
  String toJson() => toString();

  /// Returns a [ClaimCareTeamRoleCodes] from a [String] enum.
  static ClaimCareTeamRoleCodes fromString(String str) {
    switch (str) {
      case 'primary':
        return ClaimCareTeamRoleCodes.primary;
      case 'assist':
        return ClaimCareTeamRoleCodes.assist;
      case 'supervisor':
        return ClaimCareTeamRoleCodes.supervisor;
      case 'other':
        return ClaimCareTeamRoleCodes.other;
      default:
        throw ArgumentError('Unknown enum value: $str');
    }
  }

  /// Returns a [ClaimCareTeamRoleCodes] from a json [String] (although it will accept any dynamic and throw an error if it is not a String due to requirements for serializing/deserializing
  static ClaimCareTeamRoleCodes fromJson(dynamic jsonValue) {
    if (jsonValue is String) {
      return fromString(jsonValue);
    } else {
      throw ArgumentError('Unknown enum value: $jsonValue');
    }
  }
}
