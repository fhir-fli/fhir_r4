...
class ValidationLocation {
  /// Constructor for the [ValidationLocation] class.
  ValidationLocation(this.start, this.end, [this.source = '']);

  /// Creates a new location instance.
  factory ValidationLocation.create(
    int startLine,
    int startColumn,
    int startOffset,
    int endLine,
    ...
    return ValidationLocation(startSegment, endSegment, source);
  }
  ...
  String toString() {
    ...
    return 'ValidationLocation($start, $end$src)';
  }
}
...
/// Tokenizes a JSON string into a list of tokens.
String astSubstring(String str, int start, [int? end]) {
  end ??= start + 1;
    ...
    switch (state) {
      ...
      case _ObjectState.OPEN_OBJECT:
        if (token.type == TokenType.RIGHT_BRACE) {
          if (settings.loc) {
            ...
            object.loc = ValidationLocation.create(
      ...
      case _ObjectState.PROPERTY:
        if (token.type == TokenType.RIGHT_BRACE) {
          ...
          final src = settings.source ?? '';
          final msg = unexpectedToken(
            astSubstring(input, token.loc!.start.offset, token.loc!.end.offset),
            src,
            token.loc!.start.line,
            token.loc!.start.column,
          );
  ...
  while (index < tokenList.length) {
    ...
    switch (state) {
      ...
      case _PropertyState.COLON:
        ...
        final src = settings.source ?? '';
        property.loc = ValidationLocation.create(
          startToken.loc!.start.line,
          startToken.loc!.start.column,
          startToken.loc!.start.offset,
    ...
    switch (state) {
      ...
      case _ArrayState.OPEN_ARRAY:
        if (token.type == TokenType.RIGHT_BRACKET) {
          final src = settings.source ?? '';
          array.loc = ValidationLocation.create(
            startToken.loc!.start.line,
            startToken.loc!.start.column,
            startToken.loc!.start.offset,
            token.loc!.end.line,
...
List<Token> tokenize(String input, Settings settings) {
  ...
  while (index < input.length) {
    ...
    if (token != null) {
      final src = settings.source ?? '';
      token.loc = ValidationLocation.create(
        line,
        column,
        index,
        token.line,
      ...
      final msg = unexpectedSymbol(
        astSubstring(input, index, index + 1),
        src,
        line,
        column,
      );
